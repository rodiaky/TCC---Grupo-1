...
<main class="grid-geral">
    ...
    <section class="selecionarFiltro sectionCinza">
        ...
        <form action="{{ route('admin.questoes.filtrar') }}" method="GET" class="filtros">
            ...
            <div class="filtro-item selecionar-seta">
                <select name="id_banca" id="banca" class="selecionar">
            ...
            <div class="filtro-item selecionar-seta">
                <select name="assunto" id="assunto" class="selecionar">
                    <option value="">Selecione o Assunto</option>
                    <hr>
                    <option value="">Todos os Assuntos</option>
                    <hr>
                    @foreach($assuntos as $assunto)
                        <option value="{{ $assunto }}" {{ request('assunto') == $assunto ? 'selected' : '' }}>{{ $assunto }}
                        </option>
                    @endforeach
                </select>
            </div>
                        ...
                        @if ($isAdmin)
                            <button type="button" class="botao-editar">
                                ...
                                <div class="editar-opcoes">
                                    <a href="{{ route('admin.questoes.editar', $questao->id) }}"
                                        class="editar-opcoes-texto">Editar</a>
                                    <hr>
                                    <a href="{{ route('admin.questoes.excluir', $questao->id) }}"
                                        class="editar-opcoes-texto">Excluir</a>
                                </div>
                            </button>
                        @endif
                    </div>
                    <div class="assunto">{{ $questao->assunto }}</div>
                    <div class="texto">
                ...
                @for ($i = 1; $i <= $questoes->lastPage(); $i++)
                    ...
                    @else
                        <a class="pagination-link"
                            href="{{ $questoes->url($i) . '&' . http_build_query(request()->except('page')) }}">{{ $i }}</a>
                    @endif
                @endfor
            </div>
        </div>
